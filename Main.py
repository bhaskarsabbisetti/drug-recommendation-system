...
def recommendDrug():
    ...
    for i in range(len(testData)):
        disease = testData.iloc[i, 0].strip().lower()

        # Search for best matching condition
        match_scores = []
        for j in range(len(condition)):
            if isinstance(condition[j], str):
                sim = 1 if disease in condition[j].lower() else 0
                match_scores.append((sim, drug_name[j], rating[j]))
            else:
                match_scores.append((0, "none", 0))

        # Get the best match
        match_scores.sort(reverse=True)
        top_score, best_drug, predicted_rating = match_scores[0]

        text.insert(END, f"Disease Name: {disease}\n")
        text.insert(END, f"Recommended Drug: {best_drug}\n")
        text.insert(END, f"Predicted Ratings: {predicted_rating}\n\n")
